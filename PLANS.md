    ...
    - Data/計算:
      ...
      - 表示上の注意:
        ...
        - 単位は `万円` 表示（ツールチップは `円` + `万円` 併記でも可）。
          - 実装方針: ウォーターフォールの金額ラベルは原則 `万円` に統一し、`formatManYen(valueYen)`（四捨五入・必要なら小数1桁）で表示する。
            - 例: `¥1,234,567` → `123万`、`-¥98,765` → `-10万`
            - 詳細確認用に、タップ/ホバー時の補助表示（ツールチップ/選択段表示）では `円` も併記してよい（`123万（¥1,234,567）`）。
        - モバイル（幅が狭い）では、段数に対して横幅が足りず「各段の金額テキストが重なって読めない」ことがある。
          - 原因: bar/slot 幅が縮む一方、金額ラベル（例: `¥1,234,567`）は `nowrap` で描画されるため衝突する。
          - 解決策（推奨: レスポンシブ表示ポリシー）:
            1) `slotWidth`（または計測した描画幅 / 段数）を基準にラベルを出し分ける。
               - `slotWidth >= 80px`: いまの通り全段の金額を表示
               - `50px <= slotWidth < 80px`: 金額を短縮表記（`123万` / `1.2M` 等）＋フォントサイズ縮小
               - `slotWidth < 50px`: 全段の金額は非表示にして、タップ/ホバー時のツールチップ（または選択段だけ表示）へ退避
            2) 代替案: 金額ラベルを棒の「内側」に配置（上下ではなく中央寄せ）し、はみ出す場合は `…` 省略。
            3) 代替案: 金額ラベルを `-30deg` などで回転して衝突を減らす（最終手段）。
          - 受け入れ（モバイル）: 横スクロールなしでも、金額が重ならず「必要な段の金額を確認できる」こと（ツールチップ/選択表示でも可）。
    - UI/実装方針（Victory前提）:
...
42a. Make landing more “pop” and attractive (ポップで魅力的なLPデザイン改善):
    - Goal: 「かわいい/ポップ/安心」な第一印象を作り、CTA（今すぐ試す）を押したくなる見た目にする（機能は変えない）。
    - 方針:
      - 依存追加なし（アイコンは既存SVG、背景はCSSだけで表現）。
      - “派手すぎない” を優先しつつ、色・余白・イラスト風アクセントで雰囲気を上げる。
      - パフォーマンス優先: 画像を追加する場合は `public/` に軽量SVG（数KB）だけ。prefers-reduced-motion を尊重。
    - UI提案（具体）:
      - Hero背景:
        - 既存の淡いグラデに加え、ドット/波/ブロブの装飾をCSSで重ねる（`radial-gradient` 複数 + `mask` なしでOK）。
        - 右上に小さな“キラッ”バッジ（例: 「3分でわかる」）を配置。
      - CTAボタン:
        - Primaryボタンを“ピル型 + グラデ + 影”にし、ホバー/押下で軽いバウンス（`transform: translateY()` 程度）。
        - ボタン直下に「会員登録なし / あとで修正OK」のミニタグ（pill）を2〜3個並べる。
      - セクションカード:
        - “開始方法3択” をカード化し、各カードにアイコン＋1行説明＋「おすすめ」ラベル（かんたん入力のみ）。
        - 角丸・影・境界線を統一し、hoverでほんの少し浮く（モバイルはhoverなし）。
      - 結果プレビュー:
        - プレビュー枠を“ポストカード”風に（角丸 + 影 + 端のテープ表現をCSSで疑似要素）。
        - KPIは“数字を大きく、ラベルを小さく”＋カラーピルでカテゴリ色を付与。
      - Trust note:
        - “安心”アイコン＋短文（1行）にし、詳細はアコーディオンへ。
    - 実装ポイント:
      - `app/src/App.css`: `.landing-*` のテーマ変数（例: `--lp-accent`, `--lp-cta-grad`）追加、Hero背景/カード/CTAを更新。
      - `app/src/App.tsx` / `app/src/components/LandingPreview.tsx`: 既存DOMは極力維持し、クラス追加で見た目を変更（差分小）。
      - アクセシビリティ:
        - コントラスト確保、装飾は `aria-hidden`、動きは `prefers-reduced-motion` で停止。
    - Acceptance:
      - モバイル/PCで「ボタンが押したくなる」見た目（余白・色・カードの統一感）になっている。
      - `npm run lint/test/build` が通る。
